...
(Gun.chain as any).chain = function(this: any, sub?: any): any {
  const gun = this;
  const at = gun._;
...
function output(this: any, msg: GunMessage): any {
  let put: any, get: any;
  const at = this.as;
  let back = at.back;
  const root = at.root;
  ...
  if (get = msg.get) {
    ...
    if (at.lex) { 
      Object.keys(at.lex).forEach(function(k) { tmp[k] = at.lex[k]; }, tmp = msg.get = msg.get || {}); 
    }
...
function input(this: any, msg: GunMessage, cat?: any): void {
  ...
  let sat: any; // eve = event, at = data at, cat = chain at, sat = sub at (children chains).
  ...
  if (u !== msg.put && (u === tmp['#'] || u === tmp['.'] || (u === tmp[':'] && u === tmp['=']) || u === tmp['>'])) { // convert from old format
    if (!valid(tmp)) {
      ...
      return (setTimeout as any).each(Object.keys(tmp).sort(), function(k: string) { // TODO: .keys( is slow // BUG? ?Some re-in logic may depend on this being sync?
  ...
  if (cat !== at) { // don't worry about this when first understanding the code, it handles changing contexts on a message. A soul chain will never have a different context.
    ...
    } else if (at.soul) { // a has (property) chain will have a different context sometimes if it is linked (to a soul chain). Anything that is not a soul or has chain, will always have different contexts.
      tmp.$ = cat.$;
      tmp.$$ = tmp.$$ || at.$;
    }
  ...
  if (!at.soul /*&& (at.ask||'')[''] */ && state >= state_is(root.graph[soul], key) && (sat = (root.$.get(soul)._.next || '')[key])) { // Same as above here, but for other types of chains. // TODO: Improve perf by preventing echoes recaching.
    ...
    if ('string' == typeof (tmp = valid(change))) {
      sat.put = root.$.get(tmp)._.put || change; // share same cache as what we're linking to.
    }
  }

  this.to && this.to.next(msg); // 1st API job is to call all chain listeners.
  // TODO: Make input more reusable by only doing these (some?) calls if we are a chain we recognize? This means each input listener would be responsible for when listeners need to be called, which makes sense, as they might want to filter.
  cat.any && (setTimeout as any).each(Object.keys(cat.any), function(any: any) { 
  ...
  cat.echo && (setTimeout as any).each(Object.keys(cat.echo), function(lat: any) { 
  ...
  if (((msg.$$ || '')['_'] || at).soul) { // comments are linear, but this line of code is non-linear, so if I were to comment what it does, you'd have to read 42 other comments first... but you can't read any of those comments until you first read this comment. What!? // shouldn't this match link's check?
    ...
    if ((sat = cat.next) && (sat = sat[key])) { // TODO: possible trick? Maybe have `ionmap` code set a sat? // TODO: Maybe we should do `cat.ask` instead? I guess does not matter.
      tmp = {};
      Object.keys(msg).forEach(function(k) { tmp[k] = msg[k]; });
...
function link(msg: GunMessage, cat?: any): void {
  ...
  if ('string' != typeof (link = valid(link))) {
    if (this === Gun.on) { 
      (tat.echo || (tat.echo = {}))[cat.id] = cat; // allow some chain to explicitly force linking to simple data.
    }
  ...
  (setTimeout as any).each(Object.keys(tmp), function(get: string, sat: any) { // if sub chains are asking for data. // TODO: .keys( is slow // BUG? ?Some re-in logic may depend on this being sync?
...
function unlink(msg: GunMessage, cat: any): void { // ugh, so much code for seemingly edge case behavior.
  const put = msg.put || '';
  const change = (u !== put['=']) ? put['='] : put[':'];
  const root = cat.root;
  let link: any, tmp: any;
...
function ack(this: any, msg: GunMessage, ev?: any): any {
...
const empty = {};
const u = undefined;
const text_rand = (String as any).random;
const valid = Gun.valid;
const obj_has = function(o: any, k: string): boolean { 
