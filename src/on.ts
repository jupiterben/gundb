...
(Gun.chain as any).on = function(this: any, tag: any, arg?: any, eas?: any, as?: any): any { // don't rewrite!
  const gun = this;
  const cat = gun._;
  const root = cat.root;
  let act: any, off: any, id: any, tmp: any;
  ...
  let opt: OnOptions = arg;
  opt = (true === arg) ? { change: true } : opt || {};
  opt.not = 1; 
  opt.on = 1;
  //opt.at = cat;
  //opt.ok = tag;
  //opt.last = {};
  const wait: any = {}; // can we assign this to the at instead, like in once?
  gun.get(tag, opt);
...
(Gun.chain as any).once = function(this: any, cb?: any, opt?: OnceOptions): any { 
  ...
  const root = cat.root;
  const data = cat.put;
  const id = (String as any).random(7);
  let one: any;
  let tmp: any;
  
  gun.get(function(this: any, data: any, key: any, msg: any, eve: any) {
    const $ = this;
    const at = $._;
    one = (at.one || (at.one = {}));
    ...
    function once(f?: any) {
      let atLocal = at;
      if (!at.has && !at.soul) { 
        atLocal = { put: data, get: key }; // handles non-core messages.
      }
      
      if (u === (tmp = atLocal.put)) { 
        tmp = ((msg.$$ || '')['_'] || '').put; 
      }
...
function none(gun: any, opt: any, chain?: any): any {
  Gun.log.once("valonce", "Chainable val is experimental, its behavior and API may change moving forward. Please play with it and report bugs and ideas on how to improve it.");
  (chain = gun.chain())._.nix = gun.once(function(data: any, key: any) { 
...
(Gun.chain as any).off = function(this: any): any {
  ...
  if (tmp = at.map) {
    Object.keys(tmp).forEach(function(i) { 
      const atItem = tmp[i]; //obj_map(tmp, function(at){
      if (atItem.link) {
        cat.root.$.get(atItem.link).off();
      }
    });
  }
  ...
  if (tmp = at.next) {
    Object.keys(tmp).forEach(function(i) { 
      const neat = tmp[i]; //obj_map(tmp, function(neat){
      neat.$.off();
    });
  }
